...
const DistributeToSr = () => {
    ...
    if (searchedProduct === "") {
      ...
      curProductlist.forEach((item) => {
        if (item.product_name === searchedProduct) {
          if((parseInt(item.distributed_amount) + parseInt(numofPieces))<=parseInt(numberOfPieces)){            
  ...
  const handleSearchBar = (event) => {
    ...
    const searchedList = newProductpckg.filter((p) =>
      p._id.product_name.toLowerCase().includes(event.target.value.toLowerCase())
    );
  ...
  const handleSelectOption = (event) => {
    ...
    let totalNum = productList?.filter(p=>p._id.product_name===selectedOption)[0]?.count;
    ...
    .then(data=>{
      if(data.length>0){
        const prevCount = data?.filter(d=>d._id.product_name===selectedOption && d._id.sender_id===userId)[0]?.count;
